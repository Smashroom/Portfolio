...
crowd = {smallStr,bigStr};
dist = {uniStr,nonStr} ;

crossRate = [0.1,0.5,1];
mutationRate = [0.1,0.5,1];
...
for crowdIndex = 1:2
    for distIndex = 1:2
        for crossRateIndex = 1:3
            for mutationRateIndex = 1:3
                for l = 1 : 10

                    if(strcmp(crowd(crowdIndex),'SMALL'))
                    ...
                    if(strcmp(dist(distIndex),'UNIFORM'))
                    ...
                    for i = 1 : iterationSize
                        ...
                        % Mutation 
                        for j = 1 :  length(pop)
                            if(mutationRate(mutationRateIndex) < rand())
                                 continue;
                             end
                        ...
                        for m = 1 : length(nextPop)
                            if (rand() > crossRate(crossRateIndex) )
                               continue;
                            end
                ...
                strMode = [crowd(crowdIndex),' , ',dist(distIndex),' , ', 'Mutation: ',num2str(mutationRate(mutationRateIndex)), ' CrossingOver: ',num2str(crossRate(crossRateIndex))];
                figure
                subplot(1,2,1)
                plot(popBest,'r')
                title(strMode)
