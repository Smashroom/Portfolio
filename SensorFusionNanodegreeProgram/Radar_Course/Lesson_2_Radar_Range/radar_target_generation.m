...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Frequency of operation = 77GHz x 
% Max Range = 200m 
...
for i = (Gr+Tr+1):(Nr/2-(Gr+Tr))
    for j = (Gc+Tc+1):(Nd-(Gc+Tc))
        ...
        % Traverse around training cells to 
        % calculate noise threshold
        signal = RDM(i,j);
        cut_cell_noise = [];
        % Traverse rows of the training cells
        for k = -(Gr+Tr):(Gr+Tr)
            for l = -(Gc+Tc):(Gc+Tc)
                % Pass guard cells
                if (k > -Gr) && (k < Gr)
                ...
                cut_cell_noise = [cut_cell_noise db2pow(RDM(i+k, j+l))];
            end
        end
        
        noise_threshold = offset+pow2db(sum(sum(cut_cell_noise))/length(cut_cell_noise));
