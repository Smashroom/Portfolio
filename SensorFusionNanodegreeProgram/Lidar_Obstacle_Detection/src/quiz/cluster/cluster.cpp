...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
	...
	viewer->setBackgroundColor (0, 0, 0);
  	...
  	viewer->addCube(window.x_min, window.x_max, window.y_min, window.y_max, 0, 0, 0, 0.1, 0.1, "window");
  	return viewer;
}
...
void proximity_scan(const std::vector<std::vector<float>>& points, std::vector<int>* cluster, std::vector<bool>* points_visited, KdTree* tree, int point_idx, float distanceTol)
{
	...
	for(int idx: nearby){
		if(points_visited->at(idx) == false ){
			proximity_scan(points, cluster, points_visited, tree, idx, distanceTol);	
		}
	}
}
...
//	std::bitset<size_points> points_visited(0);
	std::vector <bool> points_visited(points.size(),false);
	std::vector<std::vector<int>> clusters;
	std::cout << "EC ... \n";
	
	for (auto it = points.begin(); it != points.end(); ++it) {
		int point_idx = std::distance(points.begin(), it);
		if(points_visited[point_idx] == false ){
			std::vector<int> cluster;
			std::cout << "PS ... \n";
	
			proximity_scan(points, &cluster, &points_visited, tree, point_idx, distanceTol);
