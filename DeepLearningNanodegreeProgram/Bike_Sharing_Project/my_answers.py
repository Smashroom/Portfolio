...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # Set sigmoid as an activation function for hidden layer
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        # The derivate of f(x) = x is f'(x) = 1, so the error term is:
        output_error_term = error
        
        hidden_error = output_error_term.dot(self.weights_hidden_to_output.T)
        hidden_error_term = hidden_error*hidden_outputs*(1-hidden_outputs)
        # Weight step (input to hidden)
        delta_weights_i_h += hidden_error_term.T * X[:, None]#self.weights_input_to_hidden.dot(hidden_error_term[:,None])
        
        # Weight step (hidden to output)
        delta_weights_h_o += output_error_term * hidden_outputs[:, None]#self.weights_hidden_to_output.dot(output_error_term[:,None])
